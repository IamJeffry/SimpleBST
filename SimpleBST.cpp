...
struct node* search(struct node *root, int x){
	...
	else if(x > root->data) return search(root->right_child, x);
	else return search(root->left_child, x);
}

struct node* find_minimum(struct node *root){
	if(root == NULL) return NULL;
	else if(root->left_child != NULL) return find_minimum(root->left_child);
	return root;
}
...
struct node* insert(struct node *root, int x){
	...
	else if(x > root->data) root->right_child = insert(root->right_child, x);
